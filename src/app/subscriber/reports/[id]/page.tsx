...
import { ArrowLeft, Calendar, MapPin, AlertTriangle, Bookmark, BookmarkCheck, Loader2, ImageIcon, ChevronRight, Info, Car } from 'lucide-react';
...
export default function SubscriberReportDetailPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                ...
                if (res.ok) { const data = await res.json(); setReport(data.report); } else toast.error('Report not found');
                if (user?.id) { const savedRes = await fetch('/api/saved-reports'); const savedData = await savedRes.json(); setIsSaved((savedData.savedReports || []).some((s: any) => s.report_id === params.id)); }
    ...
    const toggleSave = async () => {
        ...
        try {
            if (isSaved) { await fetch('/api/saved-reports', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ report_id: report.id }) }); setIsSaved(false); toast.success('Removed'); }
            else { await fetch('/api/saved-reports', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ report_id: report.id }) }); setIsSaved(true); toast.success('Saved!'); }
        } catch { toast.error('Failed'); }
    };

    if (loading) return <DashboardLayout role={UserRole.SUBSCRIBER}><div className="flex items-center justify-center h-96"><Loader2 className="w-8 h-8 animate-spin text-indigo-600" /></div></DashboardLayout>;
    if (!report) return <DashboardLayout role={UserRole.SUBSCRIBER}><div className="text-center py-16"><Car className="w-12 h-12 text-slate-300 mx-auto mb-4" /><h2 className="text-xl font-bold text-slate-900 mb-2">Report Not Found</h2><Link href="/subscriber/search"><Button variant="outline" className="rounded-lg"><ArrowLeft className="w-4 h-4 mr-2" />Search</Button></Link></div></DashboardLayout>;

    return (
        <DashboardLayout role={UserRole.SUBSCRIBER}>
            <FadeInView delay={0.1} className="space-y-8 max-w-6xl mx-auto">
                <div className="flex items-center gap-2 text-sm text-slate-500">
                    <Link href="/subscriber/search" className="hover:text-indigo-600 transition-colors">Search</Link>
                    <ChevronRight className="w-3.5 h-3.5" />
                    <span className="text-slate-900 font-medium">{report.vehicle_make} {report.vehicle_model}</span>
                </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <Button variant={isSaved ? "default" : "outline"} className={cn("rounded-lg h-10 px-5 font-medium text-sm gap-2", isSaved ? "bg-indigo-600 hover:bg-indigo-700 text-white" : "border-slate-200 text-slate-600 hover:bg-slate-50")} onClick={toggleSave}>
                        {isSaved ? <><BookmarkCheck className="w-4 h-4" />Saved</> : <><Bookmark className="w-4 h-4" />Save Report</>}
                    </Button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <Card className="bg-white border-slate-200 shadow-sm rounded-xl">
                            <CardHeader className="px-6 py-5 pb-3"><CardTitle className="text-base font-semibold flex items-center gap-2"><Car className="w-4 h-4 text-blue-600" />Vehicle Details</CardTitle></CardHeader>
